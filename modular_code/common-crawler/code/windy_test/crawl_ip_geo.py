...
def worker(idx, webcam):
    ...
    html_json = {"original_url": new_webcam["original_url"], "get_original_page_success": get_original_page_success, "original_page": original_page_text, "secondary_url": None, "secondary_page": None}    # if found in original_url, use it
...
if __name__ == "__main__":
    ...
    for idx, webcam in tbar:
        ...
        if (idx + 1) % 500 == 0 or (idx + 1) == total_num:
            with open(os.path.join("results", "new_all_webcams_with_ip_{}_{}.json".format(start + recorded, start + idx)), "w") as f:
