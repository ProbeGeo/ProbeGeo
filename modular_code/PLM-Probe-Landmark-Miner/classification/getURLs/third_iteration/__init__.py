...
def dataprocess1():
    ...
    all2=[]
    f = open('../../../crawling/third_iteration/Hyperlink guided search/LGinlinkurlfirstnew.jsonn', encoding='utf-8')  #
    res = f.read()  #
    data = json.loads(res)
    for key in data:
        all2.append(key)
    print(len(all2))
    f = open('../../../crawling/third_iteration/Similarity guided search/fatherURLsnew.json', encoding='utf-8')  #
    res = f.read()  #
    data = json.loads(res)
    for key in data:
        all2.append(key)
